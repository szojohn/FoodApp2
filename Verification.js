...
// Get categories
const categoriesToArr = (id, name, icon) => {
    let category = {};
    category["id"] = id;
    category["name"] = name;
    category["icon"] = icon;
...
const categoryList = () => {
    ...
    db.collection("categoryData").orderBy("id", "asc").get().then(querySnapshot => {
        querySnapshot.forEach((doc) => {
            id = doc.get("id");
            name = doc.get("name");
            icon = doc.get("icon");
            categoriesToArr(id, name, icon);
        });
    }).catch((error) => {
...
const restaurantList = () => {
    ...
    db.collection("restaurantData").orderBy("id", "asc").get().then(querySnapshot => {
        querySnapshot.forEach((doc) => {
...
// add faveCount in database
const addFave = (names, qty) => {
    if (firebase.apps.length === 0) {
        firebase.initializeApp(firebaseConfig);
    }
    const db = firebase.firestore();
    
    let nameList = names.slice()
    let qtyList = qty.slice()
    let i = 0
    let increment = firebase.firestore.FieldValue.increment(1)

    nameList.forEach((obj) => {
        db.collection(collectionName).doc(currentID).collection("favorites").where("name", "==", obj).get().then((query) => {
            if(query.size > 0) {
                increment = firebase.firestore.FieldValue.increment(qtyList[i])
                db.collection(collectionName).doc(currentID).collection("favorites").doc(query.docs[0].ref.id).update({
                    faveCount: increment
                })
            } else {
                db.collection(collectionName).doc(currentID).collection("favorites").add({
                    name: obj,
                    faveCount: qtyList[i]
                })
            }
...
function setOrders(items) {
    ...
    orderList.forEach((obj) => {
        ids.push(obj.menuId)
        names.push(obj.name)
        quantity.push(obj.qty)
        total = total + obj.total
    })

    
    db.collection(collectionName).doc(currentID).collection("orders").add({
        foodIds: ids,
        foodNames: names,
        quantity: quantity,
        totalPrice: total,
    ...
    addFave(names, quantity)
}

export {
    loginUser, registerUser, getUserFave, backReturn, clearValues, changePW, getCategoryData, categoryList,
    setOrders, categoriesData, faveArray, currentID, currentUN, currentPW,
